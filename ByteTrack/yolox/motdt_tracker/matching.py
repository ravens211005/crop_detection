...
def nearest_reid_distance(tracks, detections, metric='cosine'):
    ...
    det_features = np.asarray([track.curr_feature for track in detections], dtype=np.float64)
...
def mean_reid_distance(tracks, detections, metric='cosine'):
    ...
    track_features = np.asarray([track.curr_feature for track in tracks], dtype=np.float64)
    det_features = np.asarray([track.curr_feature for track in detections], dtype=np.float64)
